...
//! distributed Equicord plugin (see this session's chat history for the
//! "dual-purpose ELF" investigation that concluded a single-file
//! shim+installer isn't possible on glibc -- `PT_INTERP` and
//! `dlopen`/`DT_NEEDED`-loadability are mutually exclusive on one ELF
//! file, confirmed live). Ships as a second, ordinary binary alongside
//! `discord-capture-shim.so` instead.
//!
//! Equivalent to:
