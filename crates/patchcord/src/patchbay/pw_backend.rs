...
use pipewire::{context::ContextRc, keys, link::Link, main_loop::MainLoopRc, node::Node, properties::properties, types::ObjectType};
...
mod decode {
	...
	impl PodSerialize for MuteProps {
		fn serialize<O: Write + Seek>(
			...
			let mut obj = serializer.serialize_object(libspa::sys::SPA_TYPE_OBJECT_Props, libspa::sys::SPA_PARAM_Props)?;
			obj.serialize_property(libspa::sys::SPA_PROP_mute, &self.0, libspa::pod::PropertyFlags::empty())?;
			obj.end()
		}
	}

	/// Writes the serialized mute-props pod bytes into `out`.
	pub fn serialize_mute_props<O: Write + Seek>(out: O, mute: bool) -> std::result::Result<(), cookie_factory::GenError> {
...
pub enum Command {
	...
	Snapshot {
		reply: mpsc::Sender<GraphSnapshot>,
	},
...
impl PipewireBackend {
	...
	pub fn set_default_sink(&self, sink_name: Option<String>) -> Result<()> {
		let (reply_tx, reply_rx) = mpsc::channel();
		self.send(Command::SetDefaultSink {
			sink_name,
			reply: reply_tx,
		})?;
		recv_result(&reply_rx)
...
// Long by necessity: sets up the mainloop/context/core/registry, several
// shared Rc<RefCell<...>> state maps, and registers the registry's
// global/global_remove listeners as closures that all capture that shared
// state -- splitting this into helper functions would mean either passing
// the same half-dozen Rc<RefCell<...>> handles through several function
// signatures, or moving the closures out to file scope where they'd lose
// the locality that makes it clear what each one captures and why. This
// is live-verified, working event-loop wiring; restructuring it carries
// real risk for no behavior change.
...
fn run_loop(command_rx: pipewire::channel::Receiver<Command>, event_tx: mpsc::Sender<BackendEvent>) -> Result<()> {
	...
	let known_globals = std::rc::Rc::new(std::cell::RefCell::new(HashMap::<
		u32,
	...
	// announce time plus on removal, confirmed live: a node's `media.name`
	// observed via this registry-only path stayed permanently null even
	// seconds after the real live PipeWire graph (`pw-dump`) showed the
	// property clearly present. Property changes are instead delivered to
	...
	let port_owner_for_remove = port_owner;
	...
	let _global_listener = registry
		.add_listener_local()
		.global(move |global| {
			known_globals_for_global
				.borrow_mut()
				.insert(global.id, std::rc::Rc::new(global.to_owned()));
			match global.type_ {
				ObjectType::Node => {
					logger::trace(&format!(
					...
					let mut record = NodeRecord {
						id: global.id,
						props: props.into_iter().map(|(k, v)| (k, miniserde::json::Value::String(v))).collect(),
						ports: Vec::new(),
					};
					...
					if let Some(existing) = state_for_global.borrow().nodes.get(&global.id) {
						record.ports.clone_from(&existing.ports);
					}
					...
					if !node_watchers_for_global.borrow().contains_key(&global.id)
						&& let Ok(node) = registry_for_node_watch.bind::<Node, _>(global)
					{
	...
	let event_tx_for_metadata = event_tx;
			...
			let listener = metadata
				...
				.property(move |_subject, key, _type, value| {
					if key == Some("default.audio.sink") {
						...
						state.borrow_mut().default_sink_name.clone_from(&sink_name);
	...
	let state_for_commands = state;
	let core_for_commands = core;
	let registry_for_commands = registry;
	let known_globals_for_commands = known_globals;
	let created_nodes_for_commands = created_nodes;
	let created_links_for_commands = created_links;
	let default_metadata_for_commands = default_metadata;
...
// One match arm per Command variant, dispatching to the pipewire core/
// registry -- length is proportional to the number of protocol commands,
// not a factoring problem; splitting each arm into its own function would
// just relocate the same amount of code behind an extra layer of
// indirection without changing what any of it does.
#[allow(clippy::too_many_lines)]
fn handle_command(
	core: &pipewire::core::CoreRc,
	registry: &pipewire::registry::RegistryRc,
	known_globals: &std::rc::Rc<
		std::cell::RefCell<HashMap<u32, std::rc::Rc<pipewire::registry::GlobalObject<pipewire::properties::PropertiesBox>>>>,
	>,
	...
	match command {
		...
		Command::DestroyLinkById { link_id, reply } => {
			...
			// success), not into_sync_result(): confirmed live that
			// destroy_global genuinely returns an asynchronous success
			// in practice (the actual object removal is confirmed later
			// via the registry's own global_remove callback, not
			// synchronously here) -- into_sync_result() panics on an
		...
		Command::SetMute { node_id, mute, reply } => {
			let result = (|| {
				...
				let global = known_globals
					.borrow()
					.get(&node_id)
					.cloned()
					.ok_or_else(|| BackendError::Message(format!("no known global for node {node_id}")))?;
				let node = registry
					.bind::<Node, _>(&*global)
					.map_err(|err| BackendError::Message(format!("failed to bind node {node_id}: {err}")))?;

				let mut vec = Vec::<u8>::new();
				decode::serialize_mute_props(Cursor::new(&mut vec), mute)
					.map_err(|err| BackendError::Message(format!("failed to build mute props pod: {err}")))?;
		...
		Command::SetDefaultSink { sink_name, reply } => {
			let result = (|| {
				...
				let metadata = metadata
					.as_ref()
					.ok_or_else(|| BackendError::Message("default metadata object not available yet".to_string()))?;
