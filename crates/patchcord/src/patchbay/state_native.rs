...
pub struct NativeSession {
    ...
    /// device detected, switch to it?" prompt, unlike the `/Virtual`
    /// class.
...
impl PatchbayStateNative {
    ...
    /// PulseAudio sink-input indices (`object.serial` -- the exact index
    /// `pa_stream_set_monitor_stream` receives and `pactl list
    /// sink-inputs` reports, NOT the PipeWire registry id
    /// `ShareableNode.id`/`discord_capture_targets` itself uses) the
    /// shim should allow `discord_capture` streams to actually monitor.
    /// See `discord-capture-shim`'s own module doc comment for the full
    /// mechanism and path-derivation this must match exactly.
    ///
    ...
    fn write_discord_capture_allowlist(&self) -> Result<()> {
        ...
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|err| {
                BackendError::Message(format!("failed to create {}: {err}", parent.display()))
            })?;
        }
        ...
        std::fs::write(&tmp_path, contents).map_err(|err| {
            BackendError::Message(format!("failed to write {}: {err}", tmp_path.display()))
        })?;
    ...
    pub fn sync_discord_capture_links(&mut self) -> Result<()> {
        ...
        let capture_node_ids: Vec<u32> = snapshot
            .nodes
            .values()
            .filter(|n| n.is_discord_capture())
            .map(|n| n.id)
            .collect();
        ...
        for capture_id in capture_node_ids {
            let Some(capture_node) = snapshot.nodes.get(&capture_id) else { continue };
            let capture_inputs = capture_node.input_ports().filter(|p| p.path.is_some()).cloned().collect::<Vec<_>>();
            if capture_inputs.is_empty() {
                logger::debug(&format!("[patchbay] discord_capture node {capture_id} has no usable input ports yet"));
                continue;
            }

            // A discord_capture node's shim-redirected link to
            // discord-capture-null (see discord-capture-shim's own doc
            // comment) can form asynchronously, independently of -- and
            // sometimes strictly after -- this exact sync running and
            // adding the real target link(s) for that same node. Because
            // a discord_capture node's real
            ...
            for &target_id in &self.session.discord_capture_targets {
                ...
                if self.is_our_virtual_audio_object(target_node) {
                    logger::warn(&format!("[patchbay] refusing to route helper-owned virtual node {target_id} into discord_capture"));
                    continue;
                }
                if !should_link(&snapshot, target_node, &self.session.discord_capture_filter, default_sink_id) {
                    logger::debug(&format!("[patchbay] node {target_id} filtered out of discord_capture routing by RouteFilter"));
                    continue;
                }
    ...
    fn remove_stray_null_sink_links(&self, snapshot: &GraphSnapshot, capture_id: u32, capture_inputs: &[models::PortRecord]) {
        ...
        for (&link_id, link) in &snapshot.links {
            ...
            let is_from_null_sink = snapshot
                .nodes
                .get(&link.output_node)
                .and_then(|n| n.prop_str("node.name"))
                == Some(DISCORD_CAPTURE_NULL_SINK_NAME);
