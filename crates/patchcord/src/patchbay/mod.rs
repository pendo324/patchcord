...
pub struct PatchbayConfig {
	...
	/// becomes the effective default (`default.audio.sink`) -- confirmed
	/// live, twice, with and without a virtual mic attached.
	///
	/// Trade-off: plain `Audio/Sink` also changes the adapter's port
	/// negotiation such that `link_monitor_to_mic`'s 2in/2out topology
	/// assumption breaks (confirmed live: mic linking silently times out).
